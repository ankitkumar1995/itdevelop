...
export async function FooterQueryKN() {
...
export async function HeaderQuery() {
  const data = await fetchAPI(`
  query MyQuery {
    page(id: "cG9zdDo0OTE4") {
      pageLogoAndPdf {
...
export async function HeaderQueryKN() {
  const data = await fetchAPI(`
  query MyQuery {
    page(id: "cG9zdDo2MDA1") {
      pageLogoAndPdf {
...
export async function MegaMenuQuery() {
  const data = await fetchAPI(`
  query MyQuery {
    menus(where: { id: 69 }) {
      nodes {
...
export async function getBoosterKitPartnerMore(first, endCur) {
  const data = await fetchAPI(
    `
query MyQuery($first:Int!,$after:String){
    page(id: "cG9zdDo0MzY4") {
      commonBanner {
        title
        subTitle
        image {
    ...
    boosterKitPartners(first: $first,after:$after) {
      nodes {
        boosterKitPartner {
          content {
            partnerEmail
...
export async function getGalleriesById(slug) {
  const data = await fetchAPI(
    `query MyQuery($id: ID!) { 
      gallery(id: $id) {
        gallery {
...
export async function getKalyanaKarnataka() {
  const data = await fetchAPI(
    `query MyQuery {
  page(id: "cG9zdDo2NDgx") {
    commonBanner {
      title
      subTitle
      image {
...
export async function getKalyanaKarnatakaKN() {
  const data = await fetchAPI(
    `query MyQuery {
  page(id: "cG9zdDo2NDgz") {
    commonBanner {
      title
      subTitle
      image {
...
export async function getGrievanceDetai() {
  const data = await fetchAPI(`
  query MyQuery {
  page(id: "cG9zdDo2NTQw") {
    raiseGrievance {
      bottomDescription
      topDescription
      image {
...
export async function getGckPreviousCalls() {
  const gckcalls = await fetchAPI(`
  query GCKQuery {
  page(id: "cG9zdDo2OTU4") {
    commonBanner {
      title
      image {
        sourceUrl
    ...
    gckPreviousCalls {
      gckCalls {
        ...
        challengeStatement
        pocsDeveloped
        startupSelectedPahse1
        startupSelectedPahse2 {
          companyName
        }
      }
    }
...
export const getkarnatakaevent = async () => {
  return await fetchAPI(`query MyQuery {
  karnatakaEventBy(slug: "elevate-karnataka") {
    elevateKarnataka {
      eventLogos {
        image {
          sourceUrl
...
export const getkarnatakaeventkn = async () => {
  return await fetchAPI(`query MyQuery {
  karnatakaEventBy(slug: "elevate-karnataka-kn") {
    elevateKarnataka {
      eventLogos {
        image {
          sourceUrl
...
export const sitkQuery = async () => {
  return await fetchAPI(`query MyQuery {
  page(id: "cG9zdDo3NDYz") {
    commonBanner {
      bannerButton {
        title
        url
