...
const EleveateForm = (props) => {
  ...
  // const [field, meta] = useField('programName');
  const [checkbox, setCheckBox] = useState(false);
  const [showModal, setShowModal] = useState(false);
  const [modalData, setModalData] = useState(null);
  const [lastStep, setLastStep] = useState(0);
  const [programName, setProgramName] = useState(
    props.eleData && props.eleData.data && props.eleData.data.companyInformation
      ? props.eleData.data.companyInformation.programName
      : ''
  );

  const initialValuesV = useMemo(() => {
    const initialValue = {
      startUpDetails: {
        founderName: '',
        designation: '',
    ...
    if (programName === 'GENERAL ELEVATE') {
      initialValue.companyInformation['scPromotedStartup'] = false;
    } else {
      initialValue.companyInformation['scPromotedStartup'] = true;
      initialValue.companyInformation['scStDetails'] = [
        {
          stakeHolderName: '',
          rdNumber: '',
          sharePercentage: '',
  ...
  const [initialValues, setInitialValues] = useState(initialValuesV);
  const getValue = (val) => {
    setCheckBox(val);
  };
  ...
  useEffect(() => {
    ...
    const initialValue = {
      ...
      companyInformation: {
        ...
        registrationNumber:
          ...
          (props.eleData &&
            props.eleData.data &&
            props.eleData.data.companyInformation &&
            props.eleData.data.companyInformation.registrationNumber) ||
          '',
        companyPan:
          (props.eleData &&
            props.eleData.data &&
      ...
      selfCertification: {
        revUnder100CR:
          (props.data &&
            props.data.data &&
            props.data.data.revenueUnder100Cr) ||
          (props.eleData.data &&
    ...
    if (programName === 'GENERAL ELEVATE') {
      ...
      initialValue.companyInformation['scStDetails'] =
        ...
        props.eleData.data &&
        props.eleData.data.companyInformation &&
        props.eleData.data.companyInformation.scStDetails.length > 0
          ? [
              ...(props.eleData &&
                props.eleData.data &&
                props.eleData.data.companyInformation &&
  ...
  const companyInformationValidateion = useMemo(() => {
    const validation = {
    ...
    if (programName !== 'GENERAL ELEVATE' && programName !== '') {
      validation['scPromotedStartup'] = yup
        .boolean()
        .required("Please select 'yes' or 'no'");

      validation['scStDetails'] = yup.array().when('scPromotedStartup', {
        is: true,
        then: yup.array().of(
          yup.object().shape({
            stakeHolderName: yup
  ...
  return (
    <div className="elevate__form">
      <FormikStepperEl
        ...
        saveExitUrl={'/elevate-karnataka'}
        formTopText={`${activeCallData?.name} :`}
        modalData={modalData}
        showModal={showModal}
        ...
        checkbox={checkbox}
        elevate
        landingPageUrl={'/elevate-karnataka'}
        formEventHeading={`${activeCallData.name}`}
      ...
      >
          ...
          validationSchema={yup.object({
            startUpDetails: yup.object({
              founderName: yup
                ...
                .max(30, 'Founder name is too long'),
              designation: yup
                .string()
                ...
                .max(30, 'Designation max 30 character are allowed'),
              emailPrimary: yup
                .string()
        ...
        <FormikStep
          ...
          validationSchema={yup.object({
            companyInformation: yup.object(companyInformationValidateion),
          })}
        >
